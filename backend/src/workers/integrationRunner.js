...
async function ensureTable(targetPool, tableName, allRecords) {
    const { schema, table } = parseTableName(tableName);

    // Coleta TODOS os campos de TODOS os registros (union)
  ...
  `);
    ...
    if (exists.recordset[0].cnt > 0) {
        ...
        await addMissingColumns(targetPool, schema, table, allFieldNames);
        return;
    }

    // Build CREATE TABLE com TODOS os campos encontrados em todos os registros
    const cols = allFieldNames.map(k =>
...
async function addMissingColumns(targetPool, schema, table, fieldNames) {
    ...
    // fieldNames já é um array de strings (union de todos os registros)
    const allNeeded = [...fieldNames, ...metaNames];
