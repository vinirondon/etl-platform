...
async function executeTest(config) {
  ...
  const detectedFields = records.length > 0 ? Object.keys(records[0]) : [];
...
function detectArrayPaths(obj, prefix = '', depth = 0, results = []) {
  ...
  if (Array.isArray(obj)) {
    results.push({ path: prefix || '', count: obj.length, sample_keys: obj[0] ? Object.keys(obj[0]).slice(0, 8) : [] });
    return results;
  }
  for (const [key, val] of Object.entries(obj)) {
    const p = prefix ? `${prefix}.${key}` : key;
    if (Array.isArray(val)) {
      results.push({ path: p, count: val.length, sample_keys: val[0] ? Object.keys(val[0]).slice(0, 8) : [] });
